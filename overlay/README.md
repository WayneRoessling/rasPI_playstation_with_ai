...
This directory holds everything the console needs beyond the plain
voice assistant: the protocol spec, the browser simulator, the RP2040
firmware, the scenario narratives + canonical catalog, the scenario
runtime, and the tools and tests that keep them in sync.
`voice_pipeline.py` at the repo root switches to this path when
`MINI_AI_OVERLAY=true`.
...
2. [`canonical/`](canonical/) — the canonical catalogs (switches, LEDs,
   displays, sound slots, tools, scenarios, hardware). Start with
   [`sfx_bank.yaml`](canonical/sfx_bank.yaml), which defines the 10 universal
   SFX slots every scenario shares.
...
│       (each: narrative.md + CHANGELOG.md + _emitted/scenario.yaml,
│        the last generated by tools/emit_scenarios.py — CI checks it's in sync)
├── tools/
...
│   └── examples/                   ✓ example scenes (all run in CI, mock mode)
│       ├── test_console_smoke.yaml
│       ├── space_command_armgate.yaml
│       ├── space_command_hold.yaml
...
│   ├── space_command_launch.py     ✓ ┐
│   ├── spaceship_cockpit.py        ✓ │
│   ├── pirate_ship.py              ✓ │ the six authored scenarios: emitted
│   ├── mars_control_normal.py      ✓ │ YAML + 3 scenario-specific tools each
│   ├── mars_control_disaster.py    ✓ │
│   ├── army_battle_command.py      ✓ ┘
│   ├── demo.py                     ✓ `python -m overlay.scenario.demo`
│   ├── test_ptt_smoke.py           ✓ PTT recording + HAL reconnect smoke test
│   ├── requirements.txt            ✓
│   └── README.md                   ✓
└── dev_assets/sfx_preview/         output of tools/generate_sfx.py
```

✓ = present. Planned work is listed under "What's NOT yet shipped" below.
...
## Narrative-first authoring (OLENT)

Scenarios are authored the "OLENT" way: **narrative first**, then a
**canonical entity catalog**. This subsystem applies just those two ideas
from the OLENT method, self-contained — it needs none of OLENT's own
tooling:

1. Write the scenario as prose with YAML frontmatter
   (`narratives/scenarios/<id>/narrative.md`), recording every change in
   that scenario's `CHANGELOG.md`.
2. `tools/emit_scenarios.py` (~200 lines) turns the frontmatter into
   `_emitted/scenario.yaml`, which the Python scenario module loads.
3. `tools/validate_canonical.py` cross-checks every scenario against the
   catalogs in `canonical/` (static check).
4. The scripted scenes in `scenes/` drive the scenarios against the
   browser simulator and assert what the panel did (behavioural check).

All of 2-4 run in CI on every pull request.
